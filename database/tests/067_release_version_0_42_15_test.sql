\echo 'Teste Migration 067_release_version_0_42_15.sql ...'
...
BEGIN
    ...
    FROM application_versions WHERE version = '0.42.15';

    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.42.15 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.42.15 steht auf %', eigene;
    END IF;

    SELECT release_status INTO vorgaenger
    FROM application_versions WHERE version = '0.42.14';

    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.42.14 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.42.15' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.42.15 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 067_release_version_0_42_15.sql ist fachlich abgenommen.'
