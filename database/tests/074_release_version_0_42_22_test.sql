\echo 'Teste Migration 074_release_version_0_42_22.sql ...'
...
BEGIN
    ...
    FROM application_versions WHERE version = '0.42.22';

    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.42.22 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.42.22 steht auf %', eigene;
    END IF;
    ...
    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.42.21 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.42.22' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.42.22 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 074_release_version_0_42_22.sql ist fachlich abgenommen.'
