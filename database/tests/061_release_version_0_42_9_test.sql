\echo 'Teste Migration 061_release_version_0_42_9.sql ...'
...
BEGIN
    ...
    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.42.9 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.42.9 steht auf %', eigene;
    END IF;

    SELECT release_status INTO vorgaenger
    FROM application_versions WHERE version = '0.42.8';

    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.42.8 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.42.9' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.42.9 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 061_release_version_0_42_9.sql ist fachlich abgenommen.'
