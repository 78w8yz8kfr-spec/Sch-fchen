\echo 'Teste Migration 083_release_version_0_42_28.sql ...'
...
BEGIN
    ...
    FROM application_versions WHERE version = '0.42.28';

    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.42.28 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.42.28 steht auf %', eigene;
    END IF;

    SELECT release_status INTO vorgaenger
    FROM application_versions WHERE version = '0.42.27';

    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.42.27 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.42.28' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.42.28 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 083_release_version_0_42_28.sql ist fachlich abgenommen.'
