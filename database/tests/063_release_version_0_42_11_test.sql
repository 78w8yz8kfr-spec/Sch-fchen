\echo 'Teste Migration 063_release_version_0_42_11.sql ...'
...
BEGIN
    ...
    FROM application_versions WHERE version = '0.42.11';

    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.42.11 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.42.11 steht auf %', eigene;
    END IF;

    SELECT release_status INTO vorgaenger
    FROM application_versions WHERE version = '0.42.10';

    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.42.10 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.42.11' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.42.11 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 063_release_version_0_42_11.sql ist fachlich abgenommen.'
