\echo 'Teste Migration 053_release_version_0_42_4.sql ...'
...
BEGIN
    ...
    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.42.4 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.42.4 steht auf %', eigene;
    END IF;

    SELECT release_status INTO vorgaenger
    FROM application_versions WHERE version = '0.42.3';

    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.42.3 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.42.4' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.42.4 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 053_release_version_0_42_4.sql ist fachlich abgenommen.'
