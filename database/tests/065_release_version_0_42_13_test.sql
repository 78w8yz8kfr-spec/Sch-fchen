\echo 'Teste Migration 065_release_version_0_42_13.sql ...'
...
BEGIN
    ...
    FROM application_versions WHERE version = '0.42.13';

    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.42.13 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.42.13 steht auf %', eigene;
    END IF;
    ...
    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.42.12 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.42.13' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.42.13 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 065_release_version_0_42_13.sql ist fachlich abgenommen.'
