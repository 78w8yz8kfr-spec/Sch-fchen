\echo 'Teste Migration 072_release_version_0_42_20.sql ...'
...
BEGIN
    ...
    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.42.20 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.42.20 steht auf %', eigene;
    END IF;
    ...
    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.42.19 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.42.20' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.42.20 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 072_release_version_0_42_20.sql ist fachlich abgenommen.'
