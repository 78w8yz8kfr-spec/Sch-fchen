\echo 'Teste Migration 157_release_version_0_44_45.sql ...'
...
BEGIN
    ...
    FROM application_versions WHERE version = '0.44.45';
    IF stand IS NULL OR stand NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.44.45 fehlt oder besitzt einen ungültigen Status';
    END IF;

    SELECT release_status INTO vorgaenger
    FROM application_versions WHERE version = '0.44.44';
    IF vorgaenger IS DISTINCT FROM 'superseded' THEN
        RAISE EXCEPTION 'Die Vorgängerfassung 0.44.44 wurde nicht korrekt abgelöst';
    END IF;
    ...
    -- herein.
    IF (
        SELECT COUNT(*) FROM application_versions WHERE version = '0.44.45'
    ) <> 1 THEN
        RAISE EXCEPTION 'Die Fassung 0.44.45 steht nicht genau einmal im Verzeichnis';
    END IF;
END;
$$;

\echo 'Migration 157_release_version_0_44_45.sql ist fachlich abgenommen.'
