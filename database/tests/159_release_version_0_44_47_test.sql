\echo 'Teste Migration 159_release_version_0_44_47.sql ...'
...
BEGIN
    ...
    FROM application_versions WHERE version = '0.44.47';
    IF stand IS NULL OR stand NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.44.47 fehlt oder besitzt einen ungültigen Status';
    END IF;
    ...
    IF vorgaenger IS DISTINCT FROM 'superseded' THEN
        RAISE EXCEPTION 'Die Vorgängerfassung 0.44.46 wurde nicht korrekt abgelöst';
    END IF;
    ...
    -- herein.
    IF (
        SELECT COUNT(*) FROM application_versions WHERE version = '0.44.47'
    ) <> 1 THEN
        RAISE EXCEPTION 'Die Fassung 0.44.47 steht nicht genau einmal im Verzeichnis';
    END IF;
END;
$$;

\echo 'Migration 159_release_version_0_44_47.sql ist fachlich abgenommen.'
