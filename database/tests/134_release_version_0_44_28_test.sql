\echo 'Teste Migration 134_release_version_0_44_28.sql ...'
...
BEGIN
    ...
    IF stand IS NULL OR stand NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.44.28 fehlt oder besitzt einen ungültigen Status';
    END IF;
    ...
    IF vorgaenger IS DISTINCT FROM 'superseded' THEN
        RAISE EXCEPTION 'Die Vorgängerfassung 0.44.27 wurde nicht korrekt abgelöst';
    END IF;

    -- Eine reine Umsortierung der Oberfläche: keine bekannte Einschränkung,
    -- und das steht ausdrücklich da statt zufällig leer zu bleiben.
    IF NOT EXISTS (
        SELECT 1 FROM application_versions
        WHERE version = '0.44.28' AND jsonb_array_length(known_issues) = 0
    ) THEN
...
\echo 'Migration 134_release_version_0_44_28.sql ist fachlich abgenommen.'
