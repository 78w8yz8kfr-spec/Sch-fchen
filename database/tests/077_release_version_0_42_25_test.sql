\echo 'Teste Migration 077_release_version_0_42_25.sql ...'
...
BEGIN
    ...
    FROM application_versions WHERE version = '0.42.25';

    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.42.25 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.42.25 steht auf %', eigene;
    END IF;
    ...
    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.42.24 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.42.25' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.42.25 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 077_release_version_0_42_25.sql ist fachlich abgenommen.'
