\echo 'Teste Migration 062_release_version_0_42_10.sql ...'
...
BEGIN
    ...
    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.42.10 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.42.10 steht auf %', eigene;
    END IF;

    SELECT release_status INTO vorgaenger
    FROM application_versions WHERE version = '0.42.9';

    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.42.9 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.42.10' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.42.10 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 062_release_version_0_42_10.sql ist fachlich abgenommen.'
