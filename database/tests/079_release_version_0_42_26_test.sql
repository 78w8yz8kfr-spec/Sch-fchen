\echo 'Teste Migration 079_release_version_0_42_26.sql ...'
...
BEGIN
    ...
    FROM application_versions WHERE version = '0.42.26';

    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.42.26 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.42.26 steht auf %', eigene;
    END IF;
    ...
    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.42.25 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.42.26' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.42.26 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 079_release_version_0_42_26.sql ist fachlich abgenommen.'
