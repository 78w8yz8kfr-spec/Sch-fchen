\echo 'Teste Migration 154_release_version_0_44_43.sql ...'
...
BEGIN
    ...
    IF stand IS NULL OR stand NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.44.43 fehlt oder besitzt einen ungültigen Status';
    END IF;
    ...
    IF vorgaenger IS DISTINCT FROM 'superseded' THEN
        RAISE EXCEPTION 'Die Vorgängerfassung 0.44.42 wurde nicht korrekt abgelöst';
    END IF;
    ...
    -- herein.
    IF (
        SELECT COUNT(*) FROM application_versions WHERE version = '0.44.43'
    ) <> 1 THEN
        RAISE EXCEPTION 'Die Fassung 0.44.43 steht nicht genau einmal im Verzeichnis';
    END IF;
END;
$$;

\echo 'Migration 154_release_version_0_44_43.sql ist fachlich abgenommen.'
