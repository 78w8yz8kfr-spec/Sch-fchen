\echo 'Teste Migration 093_release_version_0_43_1.sql ...'
...
BEGIN
    ...
    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.43.1 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.43.1 steht auf %', eigene;
    END IF;
    ...
    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.43.0 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.43.1' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.43.1 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 093_release_version_0_43_1.sql ist fachlich abgenommen.'
