\echo 'Teste Migration 150_release_version_0_44_41.sql ...'
...
BEGIN
    ...
    IF stand IS NULL OR stand NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.44.41 fehlt oder besitzt einen ungültigen Status';
    END IF;
    ...
    IF vorgaenger IS DISTINCT FROM 'superseded' THEN
        RAISE EXCEPTION 'Die Vorgängerfassung 0.44.40 wurde nicht korrekt abgelöst';
    END IF;
    ...
    -- herein.
    IF (
        SELECT COUNT(*) FROM application_versions WHERE version = '0.44.41'
    ) <> 1 THEN
        RAISE EXCEPTION 'Die Fassung 0.44.41 steht nicht genau einmal im Verzeichnis';
    END IF;
END;
$$;

\echo 'Migration 150_release_version_0_44_41.sql ist fachlich abgenommen.'
