\echo 'Teste Migration 140_release_version_0_44_34.sql ...'
...
BEGIN
    ...
    IF stand IS NULL OR stand NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.44.34 fehlt oder besitzt einen ungültigen Status';
    END IF;
    ...
    IF vorgaenger IS DISTINCT FROM 'superseded' THEN
        RAISE EXCEPTION 'Die Vorgängerfassung 0.44.33 wurde nicht korrekt abgelöst';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        WHERE version = '0.44.34' AND jsonb_array_length(known_issues) = 1
    ) THEN
...
\echo 'Migration 140_release_version_0_44_34.sql ist fachlich abgenommen.'
