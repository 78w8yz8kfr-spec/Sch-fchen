\echo 'Teste Migration 155_release_version_0_44_44.sql ...'
...
BEGIN
    ...
    IF stand IS NULL OR stand NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.44.44 fehlt oder besitzt einen ungültigen Status';
    END IF;

    SELECT release_status INTO vorgaenger
    FROM application_versions WHERE version = '0.44.43';
    IF vorgaenger IS DISTINCT FROM 'superseded' THEN
        RAISE EXCEPTION 'Die Vorgängerfassung 0.44.43 wurde nicht korrekt abgelöst';
    END IF;
    ...
    -- herein.
    IF (
        SELECT COUNT(*) FROM application_versions WHERE version = '0.44.44'
    ) <> 1 THEN
        RAISE EXCEPTION 'Die Fassung 0.44.44 steht nicht genau einmal im Verzeichnis';
    END IF;
END;
$$;

\echo 'Migration 155_release_version_0_44_44.sql ist fachlich abgenommen.'
