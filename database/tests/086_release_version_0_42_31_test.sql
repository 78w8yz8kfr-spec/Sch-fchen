\echo 'Teste Migration 086_release_version_0_42_31.sql ...'
...
BEGIN
    ...
    FROM application_versions WHERE version = '0.42.31';

    IF eigene IS NULL THEN
        RAISE EXCEPTION 'Die Fassung 0.42.31 fehlt';
    END IF;
    IF eigene NOT IN ('production', 'superseded') THEN
        RAISE EXCEPTION 'Die Fassung 0.42.31 steht auf %', eigene;
    END IF;
    ...
    IF vorgaenger IS NULL THEN
        RAISE EXCEPTION 'Die Vorgaengerfassung 0.42.30 fehlt';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE version = '0.42.31' AND rollout_percent = 100
    ) THEN
        RAISE EXCEPTION 'Die Fassung 0.42.31 wird nicht vollstaendig ausgerollt';
    END IF;
END;
$$;

\echo 'Migration 086_release_version_0_42_31.sql ist fachlich abgenommen.'
