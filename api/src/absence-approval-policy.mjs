...
export async function absenceApprovalPolicy(client, context) {
  const result = await client.query(
    `SELECT row_version, reviewer_ids, approver_ids FROM absence_approval_policies
     WHERE company_id=$1 ORDER BY row_version DESC LIMIT 1`, [context.companyId]);
  const row = result.rows[0];
  return { rowVersion: Number(row?.row_version || 0), mode: row?.reviewer_ids ? 'selected' : 'default',
    reviewerIds: row?.reviewer_ids || [], approverIds: row?.approver_ids || [] };
}

export function validateApprovalPolicy(body) {
  const fields = ['mode', 'reviewerIds', 'approverIds', 'rowVersion', 'reason'];
  ...
  if (body.mode === 'selected' && (!reviewerIds.length || !approverIds.length
      || new Set([...reviewerIds, ...approverIds]).size < 2)) {
    throw new InputError('Für beide Stufen mindestens eine Person wählen; insgesamt sind zwei verschiedene Personen nötig.');
  }
  ...
  return { ...body, reviewerIds, approverIds, reason: body.reason.trim() };
}
...
export async function saveApprovalPolicy(client, context, body) {
  ...
  await client.query(`INSERT INTO absence_approval_policies(company_id,row_version,reviewer_ids,approver_ids,actor_user_id,reason)
    VALUES($1,$2,$3,$4,$5,$6)`, [context.companyId,current.rowVersion+1,
    input.mode==='selected'?input.reviewerIds:null,input.mode==='selected'?input.approverIds:null,context.userId,input.reason]);
