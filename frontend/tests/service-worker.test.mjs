...
class FakeCache {
  ...
  async match(request, options = {}) {
    ...
    // nach app.js?v=0.44.48 die abgelegte app.js?v=0.42.1.
...
test("Installation legt genau die App-Shell im Versionscache ab", async () => {
  ...
  for (const required of [
    "./", "./index.html", "./design-system.css?v=0.44.48", "./app.js?v=0.44.48",
    "./core/device-management.js?v=0.44.48", "./core/apprentice-view.js?v=0.44.48",
    "./vendor/qr-scanner.min.js?v=0.44.48",
...
test("Ein vor dem Update gesichertes Dokument bleibt offline auffindbar", async () => {
  // Die App-Fassung steht seit 0.44.48 im Adressteil, damit ein Pflichtupdate
  ...
  const response = await runFetch(
    worker,
    requestFor(`${documentPath}?offlineScope=${scope}&appVersion=0.44.48`)
  );
...
test("Statische Dateien kommen zuerst aus dem Cache und werden sonst nachgeladen", async () => {
  ...
  const shell = await worker.caches.open(CACHE_NAME);
  const cachedRequest = requestFor("/styles.css?v=0.44.48");
...
test("Waehrend einer Veroeffentlichung springt die vorherige Fassung ein", async () => {
  ...
  const neu = requestFor("/app.js?v=0.44.48");
...
test("Eine Fehlerseite des anlaufenden Dienstes ersetzt keine Datei", async () => {
  ...
  const antwort = await runFetch(worker, requestFor("/styles.css?v=0.44.48"));
