...
class FakeCache {
  ...
  async match(request, options = {}) {
    ...
    // nach app.js?v=0.42.35 die abgelegte app.js?v=0.42.1.
...
test("Installation legt genau die App-Shell im Versionscache ab", async () => {
  ...
  assert.deepEqual(names, ["schaefchen-online-v77"]);
  const shell = await worker.caches.open("schaefchen-online-v77");
  const cachedPaths = [...shell.entries.keys()];
  for (const required of [
    "./", "./index.html", "./app.js?v=0.42.35", "./platform-admin.html", "./vde/index.html",
...
test("Aktivierung räumt alte Caches ab, behält aber die Offline-Dokumente", async () => {
  const worker = loadServiceWorker();
  await worker.caches.open("schaefchen-online-v77");
...
test("Statische Dateien kommen zuerst aus dem Cache und werden sonst nachgeladen", async () => {
  ...
  const shell = await worker.caches.open("schaefchen-online-v77");
  const cachedRequest = requestFor("/styles.css?v=0.42.35");
...
test("Waehrend einer Veroeffentlichung springt die vorherige Fassung ein", async () => {
  ...
  const neu = requestFor("/app.js?v=0.42.35");
...
test("Eine Fehlerseite des anlaufenden Dienstes ersetzt keine Datei", async () => {
  ...
  const antwort = await runFetch(worker, requestFor("/styles.css?v=0.42.35"));
