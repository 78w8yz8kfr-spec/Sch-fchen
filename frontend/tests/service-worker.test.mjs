...
// Die Cache-Fassung wandert bei jeder Veroeffentlichung weiter. Sie hier noch
// einmal hinzuschreiben hiesse, zehn Stellen im Test bei jeder Anhebung
// nachzuziehen - und genau daran ist er beim Sprung auf 0.44.40 zerbrochen,
// obwohl am Verhalten des Dienst-Workers nichts falsch war. Gelesen wird sie
// deshalb aus der Quelle: geprueft wird das Verhalten, nicht die Zahl.
const CACHE_NAME = /const CACHE_NAME = "([^"]+)"/.exec(workerSource)?.[1];
assert.ok(CACHE_NAME, "sw.js nennt keinen CACHE_NAME");
...
class FakeCache {
  ...
  async match(request, options = {}) {
    ...
    // nach app.js?v=0.44.46 die abgelegte app.js?v=0.42.1.
...
test("Installation legt genau die App-Shell im Versionscache ab", async () => {
  ...
  assert.deepEqual(names, [CACHE_NAME]);
  const shell = await worker.caches.open(CACHE_NAME);
  const cachedPaths = [...shell.entries.keys()];
  for (const required of [
    "./", "./index.html", "./design-system.css?v=0.44.46", "./app.js?v=0.44.46",
    "./core/device-management.js?v=0.44.46", "./core/apprentice-view.js?v=0.44.46",
    "./vendor/qr-scanner.min.js?v=0.44.46",
...
test("Aktivierung räumt alte Caches ab, behält aber die Offline-Dokumente", async () => {
  const worker = loadServiceWorker();
  await worker.caches.open(CACHE_NAME);
...
test("Ein vor dem Update gesichertes Dokument bleibt offline auffindbar", async () => {
  // Die App-Fassung steht seit 0.44.46 im Adressteil, damit ein Pflichtupdate
  ...
  const response = await runFetch(
    worker,
    requestFor(`${documentPath}?offlineScope=${scope}&appVersion=0.44.46`)
  );
...
test("Statische Dateien kommen zuerst aus dem Cache und werden sonst nachgeladen", async () => {
  ...
  const shell = await worker.caches.open(CACHE_NAME);
  const cachedRequest = requestFor("/styles.css?v=0.44.46");
...
test("Waehrend einer Veroeffentlichung springt die vorherige Fassung ein", async () => {
  ...
  const neu = requestFor("/app.js?v=0.44.46");
...
test("Eine Fehlerseite des anlaufenden Dienstes ersetzt keine Datei", async () => {
  ...
  const antwort = await runFetch(worker, requestFor("/styles.css?v=0.44.46"));
