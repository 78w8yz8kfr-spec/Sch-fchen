import {
  ...
  greetingForHour,
  localDateKey
} from "./core/work-time.js?v=0.44.34";
import { serverIsNewer } from "./core/versions.js?v=0.44.34";
import {
  buildReportPayload,
...
} from "./core/sync-queue.js?v=0.44.34";
import {
  ...
  sessionRoles
} from "./core/permissions.js?v=0.44.34";
import {
  COMPANY_STORAGE_KEY,
  ONLINE_STORAGE_KEY,
  carriedOverMessage,
  ...
  serializeState,
  storageKey
} from "./core/state-store.js?v=0.44.34";
import { createDeviceModule } from "./core/device-management.js?v=0.44.34";
import { baustellenAusEinsaetzen, createStockModule } from "./core/stock-module.js?v=0.44.34";
import { materialBestand, materialBestandText } from "./core/stock-management.js?v=0.44.34";
import { apprenticeTodayPrompt } from "./core/apprentice-view.js?v=0.44.34";
...
(() => {
  ...
  // unter dem Anlegeformular - zweitausend Bildpunkte tief. Dass heute sieben
  // Mitarbeiter ohne Einsatz sind, ist aber das Erste, was ein Disponent
  // wissen will, und nicht das Letzte. Sie zieht deshalb an den Anfang, vor
  // die Plantafel: erst der Stand von heute, dann der Plan der Woche.
  elements.assignmentPlanningContent.append(
    elements.adminWeek,
    elements.assignmentEditForm,
    elements.assignmentPanel
  );
  // Vor die Leiste und nicht dahinter: „Wochenplan/Monatsplan" gehoert zur
  // Plantafel darunter, und ein Kennzahlenblock dazwischen risse den Reiter
  // von dem los, was er schaltet.
  elements.assignmentPlanningContent
    .querySelector(".planning-view-bar")
    .before(elements.dispatchSummary);
  ...
  function browserFileUrl(path) {
    return `${path}${path.includes("?") ? "&" : "?"}appVersion=0.44.34`;
  }
  ...
  async function downloadFile(path, fallbackName) {
    ...
    try {
      response = await fetch(path, {
        credentials: "include",
        headers: { "X-Schaefchen-Version": "0.44.34" }
      });
    } catch {
      const error = new Error("Der Server ist momentan nicht erreichbar.");
      error.network = true;
  ...
  function configureModeCopy() {
    ...
    elements.loginFooter.textContent = `Einfach vor komplex · Version 0.44.34 ${demoMode ? "Demo" : "Online"}`;
  ...
  //
  // Das Dokument laedt "app.js?v=0.44.34". Der Dienst-Worker darf im Notfall
